...
describe("type suppression inventory", () => {
  ...
  it("keeps unchecked any casts at zero and negative type assertions explicit", () => {
    ...
    expect(
      ...
      "src/plugins/registry.diagnostics.test.ts:66:@ts-expect-error JavaScript plugins may omit the required supplement builder.",
      "src/plugins/registry.diagnostics.test.ts:68:@ts-expect-error JavaScript plugins may omit the required hosted-media resolver.",
      "src/plugins/registry.diagnostics.test.ts:70:@ts-expect-error Unknown JavaScript hook names must produce a diagnostic.",
      "src/plugins/registry.diagnostics.test.ts:312:@ts-expect-error Untyped hook input reaches the existing rejection/coercion path.",
      "src/plugins/registry.diagnostics.test.ts:346:@ts-expect-error Closed registration must stop before coercing untyped hook input.",
    ]);
  });
});
